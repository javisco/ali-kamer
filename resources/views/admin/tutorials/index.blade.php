...
<div class="min-h-screen bg-slate-50 py-6 px-3 sm:px-6">
    <div class="max-w-7xl mx-auto space-y-6">
            ...
            <div class="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                ...
                <div>
                    <a href="{{ route('admin.tutorials.create') }}"
                       class="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-accent-500 hover:bg-accent-600 active:scale-98 text-slate-900 text-xs font-black shadow-xs uppercase tracking-wider transition">
        ...
        @if(session('error'))
            <div class="bg-danger/10 border border-danger/20 text-danger rounded-xl px-4 py-3 text-xs sm:text-sm font-bold flex items-center gap-2.5 shadow-xs">
                <svg class="w-5 h-5 shrink-0 text-danger" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-4 shadow-xs">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-[10px] uppercase tracking-wider font-black text-slate-400">Total</p>
                        <p class="text-2xl font-black text-slate-900 mt-0.5">{{ $totalTutorials }}</p>
                    </div>
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-4 shadow-xs">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-[10px] uppercase tracking-wider font-black text-slate-400">Publiés</p>
                        <p class="text-2xl font-black text-primary-600 mt-0.5">{{ $publishedTutorials }}</p>
                    </div>
                    <div class="w-10 h-10 rounded-xl bg-primary-600/10 text-primary-600 flex items-center justify-center">
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="bg-white border border-slate-200 rounded-2xl p-4 shadow-xs">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-[10px] uppercase tracking-wider font-black text-slate-400">Brouillons</p>
                        <p class="text-2xl font-black text-slate-900 mt-0.5">{{ $draftTutorials }}</p>
                    </div>
        ...
        @forelse($tutorials as $category => $items)
            <section class="space-y-3">
                <div class="flex items-center gap-2">
                    <span class="w-1.5 h-5 rounded-full bg-primary-600"></span>
                                    ...
                                    <tr class="hover:bg-primary-600/5 transition">
                                        ...
                                        <td class="px-5 py-4">
                                            @if($tutorial->type === 'video')
                                                <span class="inline-flex items-center gap-1 text-xs font-bold text-primary-600">
                                                    🎬 Vidéo
                                                </span>
                                            @else
                                        ...
                                        <td class="px-5 py-4">
                                            @if($tutorial->is_published)
                                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-primary-600/10 border border-primary-600/20 text-primary-600 text-[10px] font-black uppercase">
                                                    <span class="w-1.5 h-1.5 rounded-full bg-primary-600"></span>
                                                    Publié
                                                </span>
                                            @else
                                                <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-slate-100 text-slate-500 text-[10px] font-black uppercase">
                                                    Brouillon
                                                </span>
                                            @endif
                                        </td>
                                        ...
                                        <td class="px-5 py-4">
                                            <div class="flex items-center justify-end gap-1">
                                                ...
                                                <a href="{{ route('admin.tutorials.show', $tutorial) }}"
                                                   title="Voir"
                                                   class="w-8 h-8 rounded-lg flex items-center justify-center text-slate-500 hover:text-primary-600 hover:bg-primary-600/10 transition">
                                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                ...
                                                <form method="POST" action="{{ route('admin.tutorials.toggle', $tutorial) }}">
                                                    ...
                                                    <button type="submit"
                                                            ...
                                                            class="w-8 h-8 rounded-lg flex items-center justify-center {{ $tutorial->is_published ? 'text-primary-600 hover:bg-primary-600/10' : 'text-slate-400 hover:bg-slate-100' }} transition">
                                                        @if($tutorial->is_published)
                                                            <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                                                            </svg>
                                                ...
                                                <form method="POST" action="{{ route('admin.tutorials.destroy', $tutorial) }}"
                                                    ...
                                                    <button type="submit"
                                                            title="Supprimer"
                                                            class="w-8 h-8 rounded-lg flex items-center justify-center text-danger hover:bg-danger/10 transition">
                                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        ...
                        @foreach($items as $tutorial)
                            <div class="p-4 space-y-3">
                                <div class="flex items-start justify-between gap-3">
                                    ...
                                    @if($tutorial->is_published)
                                        <span class="shrink-0 px-2 py-0.5 rounded-full bg-primary-600/10 text-primary-600 text-[9px] font-black uppercase">
                                            Publié
                                        </span>
                                    @else
                                        <span class="shrink-0 px-2 py-0.5 rounded-full bg-slate-100 text-slate-500 text-[9px] font-black uppercase">
                                            Brouillon
                                        </span>
                                    @endif
                                </div>
                                ...
                                <div class="grid grid-cols-4 gap-1.5 pt-1">
                                    <a href="{{ route('admin.tutorials.show', $tutorial) }}"
                                       class="text-center px-2 py-2 rounded-xl bg-primary-600/10 text-primary-600 text-[11px] font-bold">
                                        Voir
                                    </a>

                                    <a href="{{ route('admin.tutorials.edit', $tutorial) }}"
                                       class="text-center px-2 py-2 rounded-xl bg-slate-100 text-slate-900 text-[11px] font-bold">
                                        Modifier
                                    </a>
                                    ...
                                    <form method="POST" action="{{ route('admin.tutorials.toggle', $tutorial) }}" class="w-full">
                                        @csrf
                                        <button type="submit"
                                                class="w-full text-center px-2 py-2 rounded-xl bg-primary-600/10 text-primary-600 text-[11px] font-bold">
                                    ...
                                    <form method="POST" action="{{ route('admin.tutorials.destroy', $tutorial) }}"
                                        ...
                                        @method('DELETE')
                                        <button type="submit"
                                                class="w-full text-center px-2 py-2 rounded-xl bg-danger/10 text-danger text-[11px] font-bold">
                                            Suppr.
                                        </button>
                                    </form>
                                </div>
        ...
        @empty
            <!-- 5. AUCUN TUTORIEL -->
            <div class="bg-white border border-slate-200 rounded-2xl shadow-xs p-8 sm:p-12 text-center">
                <div class="w-16 h-16 mx-auto rounded-2xl bg-primary-600/10 text-primary-600 flex items-center justify-center mb-4">
                ...
                <p class="text-xs text-slate-500 font-medium mt-1 mb-6 max-w-sm mx-auto">
                    Créez votre premier tutoriel pour guider vos acheteurs et vendeurs sur Ali-Kamer.
                </p>

                <a href="{{ route('admin.tutorials.create') }}"
                   class="inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-accent-500 hover:bg-accent-600 text-slate-900 text-xs font-black uppercase tracking-wider shadow-xs transition">
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M12 5v14M5 12h14" />
                    </svg>
