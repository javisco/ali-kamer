                        ...
                        <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <span
                                        class="px-2.5 py-1 rounded-full text-xs font-semibold
                                         {{ $agency->is_active ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-600' }}">
                                    ...
                                    @foreach ($agency->cities as $city)
                                        <span
                                            class="px-2 py-0.5 rounded-full text-xs
                                             {{ $city->is_active ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-400 line-through' }}">
                                            {{ $city->city }}
                                        </span>
                                    @endforeach
                                </div>
                            ...
                            <div class="flex flex-col gap-2 items-end">
                                ...
                                <form method="POST" action="{{ route('admin.agencies.toggle', $agency) }}">
                                    @csrf
                                    <button
                                        class="text-xs {{ $agency->is_active ? 'text-orange-500' : 'text-emerald-600' }}
                                           hover:underline">
