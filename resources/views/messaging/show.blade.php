    ...
    <div class="bg-gray-100 min-h-screen flex flex-col font-sans">
        ...
        <div class="bg-white border-b border-gray-200 sticky top-0 z-20 shadow-sm">
            <div class="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div class="w-9 h-9 rounded-full bg-indigo-100 text-indigo-700 font-bold flex items-center justify-center uppercase text-xs">
                        {{ substr($partner, 0, 2) }}
                    </div>
                    <div>
                        <p class="font-bold text-gray-900 text-sm leading-tight">{{ $partner }}</p>
                        @if ($user->isBuyer())
                            <a href="{{ route('shop.show', $conversation->shop) }}" class="text-[11px] text-indigo-600 hover:underline">
                                Voir la boutique →
                            </a>
                        @endif
                    </div>
                ...
                <span class="text-[10px] text-gray-400 hidden sm:inline-block bg-gray-50 px-2.5 py-1 rounded-full border border-gray-100">
                            ...
                            @elseif($message->isImage())
                                <div class="relative group overflow-hidden rounded-xl">
                                    ...
                                    @if (isset($message->status))
                                        ...
                                        <span class="absolute top-2 right-2 px-2 py-0.5 text-[9px] font-bold uppercase rounded-full bg-black/50 backdrop-blur-sm text-white">
                                            {{ $message->status }}
                                        </span>
                                    @endif
                                </div>
        ...
        <div class="bg-white border-t border-gray-200 sticky bottom-0 z-10">
            <div class="max-w-3xl mx-auto px-4 py-2.5">
                ...
                <form method="POST" action="{{ route('messaging.send.text', $conversation) }}" class="flex items-center gap-2" id="chatForm">
                    @csrf
                    
                    {{-- Bouton pièce jointe unifié --}}
                    <label class="text-gray-400 hover:text-indigo-600 transition cursor-pointer p-1.5 rounded-lg hover:bg-gray-100 flex-shrink-0" title="Joindre une image ou un PDF">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.172 7l-6.586 6.586a2 2 0 102.828 2.828l6.414-6.586a4 4 0 00-5.656-5.656l-6.415 6.585a6 6 0 108.486 8.486L20.5 13" />
                        </svg>
                        <input type="file" name="attachment" class="hidden" accept="image/*,.pdf" id="attachmentInput" onchange="submitAttachmentForm()">
                    </label>

                    {{-- Champ texte principal --}}
                    <input type="text" name="body" id="messageInput" placeholder="Écrivez un message..." maxlength="2000" autocomplete="off"
                    ...
                    {{-- Bouton envoi --}}
                    <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white font-medium px-4 py-2 rounded-xl transition text-sm flex items-center justify-center flex-shrink-0">
                        <span>Envoyer</span>
                    </button>
                </form>

                {{-- Formulaire invisible pour envoi de pièce jointe sans rechargement lourd --}}
                <form id="attachmentForm" method="POST" action="{{ route('messaging.send.attachment', $conversation) }}" enctype="multipart/form-data" class="hidden">
                    @csrf
