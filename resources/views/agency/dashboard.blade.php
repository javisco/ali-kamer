...
<div class="bg-slate-50/60 min-h-screen py-8">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 bg-white p-6 rounded-2xl border border-slate-200/80 shadow-xs">
            <div class="flex items-center gap-4">
                <div class="h-14 w-14 rounded-2xl bg-gradient-to-br from-[#00843D] to-[#006B32] text-white flex items-center justify-center font-black text-2xl shadow-md shadow-emerald-900/10 shrink-0">
                    {{ strtoupper(substr($agency->name, 0, 1)) }}
                </div>
                <div>
                    <div class="flex items-center gap-2">
                        <h1 class="text-2xl font-black text-slate-900 tracking-tight">{{ $agency->name }}</h1>
                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-bold bg-emerald-50 text-[#00843D] border border-emerald-200">
                            Partenaire Agréé
                        </span>
                    </div>
            ...
            <div class="flex items-center gap-3">
                <a href="{{ route('agency.wallet') }}"
                   class="inline-flex items-center gap-2 bg-[#00843D] hover:bg-[#006B32] text-white font-bold px-5 py-2.5 rounded-xl text-sm transition shadow-sm shadow-emerald-900/15">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <div class="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-xs flex flex-col justify-between">
                <div class="flex items-center justify-between">
                    ...
                    <span class="p-2 rounded-xl bg-emerald-50 text-[#00843D]">
                ...
                <div class="mt-3">
                    <p class="text-2xl font-black text-slate-900">
                        {{ number_format($stats['total_earned'], 0, ',', ' ') }} <span class="text-xs font-bold text-slate-500">FCFA</span>
                    </p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-200/80 p-5 shadow-xs flex flex-col justify-between">
                ...
                <div class="mt-3">
                    <p class="text-2xl font-black text-[#00843D]">
                        {{ number_format($stats['wallet_available'], 0, ',', ' ') }} <span class="text-xs font-bold text-slate-500">FCFA</span>
                    </p>
                </div>
            </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-6 mb-8">
            <div class="flex items-center gap-2 mb-4">
                <span class="flex h-6 w-6 items-center justify-center rounded-full bg-[#00843D]/10 text-[#00843D] text-xs font-bold">+</span>
            ...
            @if ($agency->cities->where('is_active', true)->isEmpty())
                <div class="bg-amber-50 border border-amber-200 text-amber-800 rounded-xl p-4 text-sm flex items-center gap-3">
                    ...
                    <span>Aucune ville desservie configurée pour votre agence. Contactez l'administrateur pour en assigner.</span>
                </div>
            @else
                <form method="POST" action="{{ route('agency.counters.store') }}" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                    @csrf
                    <div>
                        ...
                        <select name="city" required
                            class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs font-semibold text-slate-800 focus:bg-white focus:border-[#00843D] focus:ring-1 focus:ring-[#00843D] transition">
                    ...
                    <div>
                        ...
                        <input type="text" name="district" required placeholder="Ex: Akwa, Mokolo..."
                            class="w-full bg-slate-50 border border-slate-200 rounded-xl px-3.5 py-2.5 text-xs font-semibold text-slate-800 placeholder-slate-400 focus:bg-white focus:border-[#00843D] focus:ring-1 focus:ring-[#00843D] transition">
                    </div>
                    ...
                    <div class="md:col-span-2 lg:col-span-4 mt-2">
                        <button class="w-full md:w-auto bg-[#00843D] hover:bg-[#006B32] text-white font-bold px-6 py-2.5 rounded-xl text-xs transition shadow-sm shadow-emerald-900/15">
        ...
        <div class="space-y-4">
            ...
            @forelse($agency->counters as $counter)
                <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs overflow-hidden transition hover:border-slate-300">
                    <div class="p-5">
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-4 border-b border-slate-100">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <h3 class="font-extrabold text-slate-900 text-base">
                                        {{ $counter->city }}
                                        @if ($counter->district)
                                            <span class="text-slate-500 font-semibold">— {{ $counter->district }}</span>
                                        @endif
                                    </h3>
                                    @if ($counter->landmark)
                            ...
                            <div class="flex items-center gap-3 self-end sm:self-center">
                                <span class="px-3 py-1 rounded-full text-xs font-extrabold tracking-wide border
                                    {{ $counter->is_active ? 'bg-emerald-50 text-[#00843D] border-emerald-200' : 'bg-red-50 text-[#CE1126] border-red-200' }}">
                                    {{ $counter->is_active ? '● Actif' : '○ Inactif' }}
                                </span>

                                <form method="POST" action="{{ route('agency.counters.toggle', $counter) }}">
                                    @csrf
                                    <button class="text-xs font-bold text-slate-500 hover:text-slate-800 transition underline decoration-slate-300">
                        ...
                        <div class="mt-4">
                            ...
                            @else
                                ...
                                <div class="bg-amber-50/50 border border-dashed border-amber-300/80 rounded-xl p-4">
                                    <div class="flex items-center gap-2 mb-3 text-amber-800">
                                        <svg class="w-4 h-4 text-amber-600 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z" />
                                        </svg>
                                        <p class="text-xs font-bold">Comptoir non opérationnel — Aucun secrétaire attribué</p>
                                    </div>

                                    <form method="POST" action="{{ route('agency.secretary.store', $counter) }}" class="space-y-3">
                                        @csrf
                                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                                            <div>
                                            ...
                                            <div>
                                                <input type="tel" name="phone" required placeholder="N° Téléphone (6XXXXXXXX)"
                                                    class="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-xs font-semibold text-slate-800 placeholder-slate-400 focus:border-[#00843D] focus:ring-1 focus:ring-[#00843D]">
                                            </div>
                                            <div>
                                        ...
                                        <button class="bg-[#00843D] hover:bg-[#006B32] text-white font-bold py-2 px-4 rounded-lg text-xs transition shadow-xs">
            ...
            @empty
                <div class="bg-white rounded-2xl border border-slate-200/80 shadow-xs p-12 text-center">
                    ...
                    <p class="text-slate-400 text-xs mt-1">Utilisez le formulaire ci-dessus pour configurer votre premier comptoir.</p>
                </div>
            @endforelse
        </div>
