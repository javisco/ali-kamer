...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 mb-8">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-5 flex items-center justify-between">
            <div>
                ...
                <p class="text-xl font-bold text-gray-900 mt-1">{{ number_format($stats['total_spent'], 0, ',', ' ') }} FCFA</p>
            </div>
    ...
    @if($activeOrder)
        <div class="bg-gradient-to-r from-blue-900 to-indigo-900 text-white rounded-2xl shadow-lg p-6 mb-8">
            <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                <div>
                    <div class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-500/20 text-blue-200 border border-blue-400/30 mb-3">
                    ...
                    <p class="text-blue-200 text-sm mt-1">
                        ...
                        Destination : <span class="font-semibold text-white">{{ $activeOrder->shipment->destination_city }}</span>
                    </p>
                </div>

                <!-- OTP Box si disponible -->
                @if($activeOrder->status === \App\Models\Order::STATUS_ARRIVED_DESTINATION || $activeOrder->status === \App\Models\Order::STATUS_AWAITING_BUYER_CONFIRMATION)
                    <div class="bg-white/10 backdrop-blur-md rounded-xl p-4 border border-white/10 text-center min-w-[220px]">
                        <p class="text-xs text-blue-200 uppercase tracking-wider">Votre Code OTP à remettre</p>
                        <p class="text-3xl font-extrabold tracking-widest text-amber-400 my-1">{{ $activeOrder->otp_code }}</p>
                        <p class="text-xs text-blue-200">À donner au guichet pour retirer le colis</p>
                    </div>
                @else
                    <a href="{{ route('buyer.orders.show', $activeOrder) }}" class="inline-flex items-center justify-center px-5 py-3 rounded-xl bg-white text-blue-900 font-semibold hover:bg-blue-50 transition shadow-sm">
                        Suivre le colis
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            ...
            <a href="{{ route('buyer.orders.index') }}" class="text-sm font-semibold text-blue-600 hover:text-blue-800">
                Voir tout →
            </a>
        </div>
                        ...
                        @foreach($recentOrders as $order)
                            <tr class="hover:bg-gray-50/50 transition">
                                ...
                                <td class="px-6 py-4">
                                    ...
                                    @endphp
                                    <span class="px-2.5 py-1 rounded-full text-xs font-medium {{ $statusClasses[$order->status] ?? 'bg-gray-100 text-gray-600' }}">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <a href="{{ route('buyer.orders.show', $order) }}" class="inline-flex items-center text-xs font-semibold text-blue-600 hover:text-blue-900 bg-blue-50 px-3 py-1.5 rounded-lg hover:bg-blue-100 transition">
                                        Détails
                                    </a>
                                </td>
                            </tr>
        ...
        @else
            <div class="p-8 text-center text-gray-500">
                ...
                <a href="{{ route('buyer.catalog.index') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-semibold hover:bg-blue-700 transition">
