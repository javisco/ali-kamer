...
@section('content')
    <div class="bg-slate-50 min-h-screen py-8">
        <div class="max-w-4xl mx-auto px-4 sm:px-6">
            ...
            @if (session('success'))
                <div
                    class="bg-emerald-50 border border-emerald-200 text-[#016837] px-5 py-3.5 rounded-2xl mb-6 text-xs font-bold flex items-center gap-2">
                    ...
                    @foreach ($orders as $order)
                        @php
                            $statusConfig = [
                                ...
                                'paid' => [
                                    'label' => 'Payé',
                                    'class' => 'bg-emerald-50 text-[#016837] border-emerald-200',
                                ],
                                'preparing' => [
                                ...
                                'awaiting_buyer_confirmation' => [
                                    'label' => 'À retirer',
                                    'class' => 'bg-amber-50 text-[#F9A01B] border-amber-200',
                                ],
                                'completed' => [
                                    'label' => 'Livré ✓',
                                    'class' => 'bg-emerald-100 text-[#016837] border-emerald-300',
                                ],
                                'auto_completed' => [
                                    'label' => 'Livré ✓',
                                    'class' => 'bg-emerald-100 text-[#016837] border-emerald-300',
                                ...
                                'disputed' => [
                                    'label' => 'Litige',
                                    'class' => 'bg-red-50 text-[#E30613] border-red-200',
                                ],
                                'cancelled' => [
                                    'label' => 'Annulée',
                        ...
                        <a href="{{ route('buyer.orders.show', $order) }}"
                            ...
                            @if ($order->order_group_id)
                                <p class="text-[10px] font-bold text-slate-400 mb-1 -mt-1">
                                    Fait partie de l'achat <a
                                        href="{{ route('buyer.orders.group.show', $order->order_group_id) }}"
                                        class="text-[#016837] underline">{{ $order->orderGroup->reference ?? '' }}</a>
                                </p>
                            @endif
                            ...
                            <div class="flex items-center justify-between mb-3 border-b border-slate-100 pb-3">
                                <div>
                                    <p
                                        class="font-black text-slate-900 text-sm tracking-tight group-hover:text-[#016837] transition-colors">
                                        {{ $order->reference }}
                                    </p>
                                ...
                                <span
                                    class="px-3 py-1 rounded-xl text-[11px] font-extrabold border uppercase tracking-wider {{ $sc['class'] }}">
                                    {{ $sc['label'] }}
                                </span>
                            </div>
                            ...
                            @if ($order->status === 'awaiting_buyer_confirmation')
                                <div
                                    class="mt-3 bg-[#F9A01B]/10 border border-[#F9A01B]/30 rounded-xl px-3.5 py-2.5 text-xs text-slate-800 font-bold flex items-center justify-between">
                            ...
                            @if ($order->status === 'awaiting_payment')
                                <div
                                    class="mt-3 bg-amber-50 border border-amber-200 rounded-xl px-3.5 py-2 text-xs text-amber-800 font-bold flex items-center gap-2">
