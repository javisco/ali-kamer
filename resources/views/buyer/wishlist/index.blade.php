    ...
    <div class="max-w-5xl mx-auto px-4">
        ...
        @if($wishlist->isEmpty())
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-10 sm:p-12 text-center">
                ...
                <div class="mx-auto w-14 h-14 rounded-2xl bg-[#E30613]/10 flex items-center justify-center mb-4">
                    <svg class="w-7 h-7 text-[#E30613]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <a href="{{ route('buyer.home') }}"
                   class="mt-5 inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-[#016837] hover:bg-[#0a542d] text-white font-bold text-sm transition shadow-sm">
        ...
        @else
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4">
                ...
                @foreach($wishlist as $item)
                    ...
                    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md hover:-translate-y-0.5 transition duration-200 group flex flex-col justify-between">
                        ...
                        <div>
                            ...
                            <a href="{{ route('product.show', $item->product) }}"
                               class="block aspect-square bg-[#F7F7F2] overflow-hidden relative">
                                ...
                                @if($item->product->hasVariants())
                                    <span class="absolute top-2 left-2 bg-[#F9A01B] text-[#0a1b12] text-[10px] font-black px-2 py-0.5 rounded-md shadow-sm">
                                        VARIANTE
                                    </span>
                                @endif
                                ...
                                <form method="POST" action="{{ route('buyer.wishlist.toggle', $item->product_id) }}" class="absolute top-2 right-2 z-10">
                                    @csrf
                                    <button type="submit" 
                                            class="w-8 h-8 rounded-full bg-white/90 shadow-sm flex items-center justify-center text-[#E30613] hover:bg-[#E30613] hover:text-white transition group/btn"
                            ...
                            <div class="p-3">
                                {{-- Titre Produit --}}
                                <a href="{{ route('product.show', $item->product) }}"
                                   class="block font-bold text-[#0a1b12] text-sm line-clamp-2 hover:text-[#016837] transition leading-snug">
                                    {{ $item->product->title }}
                                </a>
                                ...
                                <div class="mt-2">
                                    @if($item->product->hasVariants())
                                        <span class="text-[10px] font-extrabold uppercase text-[#F9A01B] block">
                                            À partir de
                                        </span>
                                        <p class="text-[#016837] font-black text-base">
