    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-10">
            ...
            <div class="lg:col-span-7 space-y-4">
                ...
                <div class="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm h-[380px] sm:h-[480px] w-full relative flex items-center justify-center p-4">
                    ...
                    <div class="absolute top-4 left-4 flex flex-col gap-2 pointer-events-none z-10">
                        @if ($product->shipping_included)
                            <span class="bg-emerald-600/90 backdrop-blur-md text-white text-[10px] font-bold px-2.5 py-1 rounded-md shadow-md uppercase tracking-wider">
                                Transport inclus
                            </span>
                        @endif
                        @if ($product->hasDiscount())
                            <span class="bg-red-600 text-white text-[10px] font-extrabold px-2.5 py-1 rounded-md shadow-md">
            ...
            <div class="lg:col-span-5 flex flex-col justify-between space-y-6">
                ...
                <div class="space-y-5 bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                    {{-- Catégorie + Titre --}}
                    <div>
                        <span class="text-xs font-bold text-blue-600 uppercase tracking-wider bg-blue-50 px-2.5 py-1 rounded-md">
                    ...
                    <div class="p-4 bg-slate-50 rounded-xl border border-slate-100 flex items-baseline gap-3 flex-wrap">
                        <span class="text-3xl font-extrabold text-emerald-600">
                            {{ number_format($product->price, 0, ',', ' ') }} <span class="text-base font-bold">FCFA</span>
                        </span>
                        @if ($product->hasDiscount())
                    ...
                    <div class="grid grid-cols-2 gap-3 text-xs">
                        <div class="p-3 bg-slate-50 rounded-xl border border-slate-100">
                            ...
                            <span class="font-bold {{ $product->availableStock() > 0 ? 'text-slate-800' : 'text-red-600' }}">
                    ...
                    <a href="{{ route('shop.show', $product->shop) }}"
                       class="flex items-center gap-3.5 p-3.5 bg-slate-50 rounded-xl border border-slate-200/80 hover:border-blue-400/80 hover:bg-white transition-all duration-200 group">
                        <div class="w-11 h-11 rounded-lg bg-blue-600 text-white font-bold text-base flex items-center justify-center uppercase shadow-sm shrink-0">
                        ...
                        <svg class="w-4 h-4 text-slate-400 group-hover:text-blue-600 group-hover:translate-x-0.5 transition" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-12">
            ...
            <div class="{{ ($product->specifications && is_array($product->specifications) && count($product->specifications) > 0) ? 'lg:col-span-2' : 'lg:col-span-3' }} bg-white rounded-2xl border border-slate-200 shadow-sm p-6">
        ...
        @if ($shopProducts->count())
            <div class="mb-12">
                <div class="flex items-center justify-between mb-5">
                    ...
                    <a href="{{ route('shop.show', $product->shop) }}" class="text-xs font-bold text-blue-600 hover:underline">
                        Voir tout →
                    </a>
                </div>
                    ...
                    @foreach ($shopProducts as $item)
                        <a href="{{ route('product.show', $item) }}"
                           class="group bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-xl hover:border-blue-400/60 hover:-translate-y-1 transition-all duration-300 flex flex-col justify-between">
                            <div>
                                <div class="w-full h-48 bg-slate-50 overflow-hidden relative p-2 flex items-center justify-center">
                                ...
                                <div class="p-3">
                                    <h3 class="text-xs font-bold text-slate-800 group-hover:text-blue-600 transition line-clamp-1">
                                        {{ $item->title }}
                                    </h3>
                                    <p class="text-sm font-extrabold text-emerald-600 mt-2">
                                        {{ number_format($item->price, 0, ',', ' ') }} <span class="text-[9px]">FCFA</span>
                                    </p>
                                </div>
                            </div>
                        </a>
                            ...
                            <div>
                                <div class="w-full h-48 bg-slate-50 overflow-hidden relative p-2 flex items-center justify-center">
                                    ...
                                    @if ($item->shipping_included)
                                        <span class="absolute top-2 left-2 bg-emerald-600/90 text-white text-[9px] font-extrabold px-2 py-0.5 rounded uppercase">
                                            Transport inclus
                                        </span>
                                    @endif
                                </div>
