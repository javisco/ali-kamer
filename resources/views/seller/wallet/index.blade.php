    ...
    <div class="min-h-screen bg-slate-50 py-6 px-3 sm:px-6">
        <div class="max-w-3xl mx-auto space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                ...
                <div class="bg-white rounded-2xl border-2 border-accent-500/30 p-5 shadow-xs">
                    <p class="text-[11px] font-bold uppercase tracking-wider text-accent-500 mb-1 flex items-center gap-1.5">
                    ...
                    <p class="text-2xl sm:text-3xl font-black text-accent-500">
                        ...
                        <span class="text-xs font-bold text-accent-500">FCFA</span>
                    </p>
                    <p class="text-xs text-accent-500/80 font-medium mt-1.5">
                        Argent réel, verrouillé jusqu'à confirmation de livraison
                    </p>
                </div>

                {{-- Solde disponible --}}
                <div class="bg-white rounded-2xl border-2 border-primary-600/30 p-5 shadow-xs">
                    <p class="text-[11px] font-bold uppercase tracking-wider text-primary-600 mb-1 flex items-center gap-1.5">
                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <p class="text-xs text-primary-600/80 font-medium mt-1.5">
            ...
            @if ($user->wallet_available >= 1000)
                <a href="{{ route('seller.wallet.withdraw') }}"
                    class="block w-full bg-accent-500 hover:bg-accent-600 active:scale-98 text-slate-900 font-black
                    py-3.5 rounded-xl text-center shadow-xs transition text-sm">
                    Demander un retrait Mobile Money
                </a>
            @else
                <div class="w-full bg-slate-200 text-slate-400 font-bold py-3.5 rounded-xl text-center text-sm cursor-not-allowed border border-slate-300/60">
            ...
            <div class="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
                <div class="px-5 py-3.5 bg-slate-50/80 border-b border-slate-200 flex items-center justify-between">
                    <h2 class="font-black text-xs uppercase tracking-wider text-slate-900">
                        Historique des transactions
                    </h2>
                </div>

                @if ($transactions->isEmpty())
                    <div class="px-5 py-12 text-center text-slate-400 text-xs">
                ...
                @else
                    <div class="divide-y divide-slate-100">
                        @foreach ($transactions as $tx)
                            ...
                            <div class="flex items-center justify-between px-5 py-3.5 hover:bg-slate-50/40 transition">
                                <div class="flex-1 min-w-0 pr-4">
                                    ...
                                    @if ($tx->note)
                                        <p class="text-[11px] text-slate-400 mt-0.5 ml-8 truncate">{{ $tx->note }}</p>
                                    @endif
                                    <p class="text-[10px] text-slate-400 mt-0.5 ml-8 font-medium">
