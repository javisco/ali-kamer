...
<div class="relative" x-data="{ open: false }" @click.outside="open = false">
    ...
    <button @click="open = !open" class="relative p-2 rounded-full hover:bg-slate-100 transition">
    ...
    <div x-show="open" x-transition
        class="absolute right-0 mt-2 w-80 max-w-[90vw] bg-white rounded-xl shadow-lg border border-slate-100 z-50"
        style="display: none;">

        <div class="flex items-center justify-between px-4 py-3 border-b border-slate-100">
            <h3 class="font-bold text-sm text-slate-800">Notifications</h3>
            <button id="notifMarkAllRead" class="text-xs text-primary-600 hover:underline">
                Tout marquer comme lu
            </button>
        </div>

        <div id="notifList" class="max-h-96 overflow-y-auto divide-y divide-slate-50">
            ...
            (function() {
                ...
                function renderList(notifications) {
                    if (notifications.length === 0) {
                        list.innerHTML = '<div class="p-4 text-center text-xs text-slate-400">Aucune notification.</div>';
                        return;
                    }
                    ...
                    list.innerHTML = notifications.map(n => `
                        <a href="${n.url ?? '#'}"
                           data-id="${n.id}"
                           class="notif-item block px-4 py-3 hover:bg-slate-50 transition ${n.read ? '' : 'bg-primary-50/50'}">
                            <div class="flex items-start gap-2.5">
                                <span class="text-lg leading-none">${n.icon}</span>
                                <div class="flex-1 min-w-0">
                                    <p class="text-xs font-semibold text-slate-800 truncate">${n.title}</p>
                                    <p class="text-[11px] text-slate-500 mt-0.5 line-clamp-2">${n.body}</p>
                                    <p class="text-[10px] text-slate-400 mt-1">${n.created_at}</p>
                                </div>
                                ${n.read ? '' : '<span class="w-2 h-2 rounded-full bg-primary-500 mt-1 flex-shrink-0"></span>'}
                            </div>
                        </a>
                    `).join('');
                ...
                function loadList() {
                    fetch(urls.index, { headers: { 'Accept': 'application/json' } })
                        ...
                        .catch(() => {
                            list.innerHTML = '<div class="p-4 text-center text-xs text-danger">Erreur de chargement.</div>';
                        });
                }
